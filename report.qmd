...
both %>%
  ...
  geom_boxplot() + labs(title = "Danceability of Top 50 Songs by Country",
              caption = "Figure 1.1: Boxplot of Danceability by Country")+
...
We might need to consolidate the results given after each graph into one paragraph for this part and just reference thee tests and graphs from this paragraph. Might make it easier to understand.
...
# Read in data 
decades <- read_csv("./GitHub/spotify_decades.csv") %>% rename(decade = tag)
...
\$H_0: \\mu\_{1950} = \\mu\_{1960} = \\mu\_{1970} = \\mu\_{1980} = \\mu\_{1990} = \\mu\_{2000} = \\mu\_{2010} = \\mu\_{2020}\$, there is no difference in mean loudness scores among the decades.
...
# load data
edy <- read.csv('./GitHub/edyPlaylist.csv')
...
\$Popularity = 7.09X\_{valence} + 1.71X\_{loudness}X\_{acousticness} - 3.52X\_{valence}X\_{instrumentalness} + 50.98\$
